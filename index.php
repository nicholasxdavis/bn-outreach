            ...
            function initializeApp() {
               ...
               function showShimmer() {
                ...
                async function loadStates() {
                    try {
                        const response = await fetch('get_business_states.php');
                        if (!response.ok) throw new Error('Could not fetch states.');
                        const states = await response.json();
                        // Parse the JSON strings within the states object
                        for (const key in states) {
                            states[key] = JSON.parse(states[key]);
                        }
                ...
                const saveState = debounce(async (id, state) => {
                    try {
                        await fetch('save_business_state.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ business_id: id, state_json: JSON.stringify(state) })
                        });
                    } catch (error) {
               ...
               async function fetchData() {
                  showShimmer();
                  try {
                     await loadStates(); // Load states from DB before fetching business data
               ...
               grid.addEventListener('change', e => {
                  ...
                  if (stateChanged) saveState(id, businessStates[id]);
               });
                ...
                grid.addEventListener('input', e => {
                    if (e.target.classList.contains('notes-textarea')) {
                        ...
                        saveState(id, businessStates[id]); 
                    }
                });
